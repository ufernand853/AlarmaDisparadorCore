...
namespace AlarmaDisparadorCore.Services
{
    public class ReglaRepository
    {
        ...
        {
            const string sqlRule = @"UPDATE dbo.reglas_alarma SET nombre = @Name, operador = @LogicOperator, mensaje = @Message, activo = @IsActive, enviar_correo = @SendEmail, email_destino = @EmailTo, intervalo_minuto = @IntervaloMinuto WHERE id_regla = @Id";
            ...
            cmd.Parameters.AddWithValue("@IntervaloMinuto", regla.IntervaloMinuto);
            cmd.ExecuteNonQuery();
        }
    }
}
